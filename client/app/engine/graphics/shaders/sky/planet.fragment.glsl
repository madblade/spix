...
Collision ray_to_sphere(vec3 start, vec3 dir, float radius)
{
    float b = 2.0 * dot(start, dir);
    float c = dot(start, start) - radius * radius;

    float det = b * b - 4.0 * c;
    if (det < 0.) return Collision(false, 0., 0.);
...
vec4 atmos_sample(vec3 pos, vec3 dir)
{
    ...
    float step_dist = length(step);
    vec3 samp = start + step * 0.5;

    vec3 rayleigh_integral = vec3(0.), mie_integral = vec3(0.);
    for (int i = 0; i < nsamples; ++i) {
        float height = length(samp) - planet_radius;
        vec3 loss_from_sun = out_scatter(samp, sun);
        vec3 loss_to_camera = out_scatter(samp, -dir);
...
void main()
{
    ...
    Collision planet_inter = ray_to_sphere(offpos, dir, planet_radius);
